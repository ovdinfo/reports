...
var Header = function() {
	...
	this.noscroll, 
	this.isAnimating,
	this.container = document.getElementById( 'prose' ),
	this.trigger = this.container.querySelector( 'button.trigger' ),
	this.header = document.getElementById( 'navigator' ),
...
Header.prototype.init = function() {
	...
	if( this.pageScroll ) {
		...
		classie.add( this.container, 'notrans' );
		classie.add( this.container, 'modify' );
	}
...
Header.prototype.disable_scroll = function(){
	var self = this,
		style = window.getComputedStyle(this.toc);
	...
	if (style.display === 'none') {classie.remove( this.header, 'visible' );}
}

Header.prototype.enable_scroll = function(){
	window.onmousewheel = document.onmousewheel = document.onkeydown = document.body.ontouchmove = null;
	classie.add( this.header, 'visible' );
}
...
Header.prototype.scrollPage = function(){
	...
	if( classie.has( this.container, 'notrans' ) ) {
		classie.remove( this.container, 'notrans' );
		return false;
	}
...
Header.prototype.toggle = function(reveal){
	...
	if( reveal ) {
		...
		this.noscroll = true;
		this.disable_scroll();
		classie.remove( this.container, 'modify' );
	}
