...
// DISPLAY BY ORGANIZATORS
  ...
  BubbleChart.prototype.display_by_group = function() {
	...
	for(i=0;i<orgs.length;i++){
		this.vis.insert("rect",":first-child").attr("class", "cell row_" + i).attr("y",148+15*i).attr("x", 50).attr("width",50 + orgs[i].name.length*5).attr("height","15").style('cursor','pointer').style('fill','#F04E23').style('opacity','0').data([i]).on('mouseover', this.mouseOverGroup).on('mouseout', this.mouseOutGroup);
