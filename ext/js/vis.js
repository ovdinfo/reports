...
// DATE DISPLAY
  ...
  BubbleChart.prototype.move_towards_date = function(alpha) {
    ...
    return function(d) {
      ...
      x = xScale(d.date2);
      y = yScale(d.value);
      d.x = d.x + (x - d.x) * (_this.damper + 0.0001);
      return d.y = d.y + (y - d.y) * (_this.damper + 0.0001);
    };
  };
...
// DISPLAY BY ORGANIZATORS
  ...
  BubbleChart.prototype.display_by_type = function() {
	...
	for(i=0;i<orgs.length;i++){
		this.vis.append("rect").attr("class", "row_" + i).attr("y",38+15*i).attr("x", 50).attr("width","230").attr("height","15").style('cursor','pointer').style('fill','#F04E23').style('opacity','0').data([i]).on('mouseover', this.mouseOverGroup).on('mouseout', this.mouseOutGroup);
  ...
  BubbleChart.prototype.display_groups = function(groups) {
    ...
    d3.selectAll('circle').on("mouseover", function(d) {
      return that.mouseOverGroup(d.inOrgID);
