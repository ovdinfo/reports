...
require([
    'jquery', 'bootstrap', 'tablesorter', 'tablesorter.widgets', 'underscore', 'spin', 'share', 'moment', 'mapbox', 'mapbox.converters.googledocs', 'goog!visualization,1,packages:[corechart],language:en'
],
...
function buildTable(data) {
  ...
  $.each(data.feed.entry, function (key, val) {
    var content = '<tr class="data ' + val.gsx$ovdid.$t + ' hider"><td>' + val.gsx$date.$t + '</td><td>' + val.gsx$ovd.$t + '</td><td>' + val.gsx$description.$t + '</td><td>' + val.gsx$ovdvalue.$t + '</td><td><a href="/2012/data/en/#' + val.gsx$ovdid.$t + '">source</a></td></tr>';
...
function drawVisualization(ovd) {
  ...
  var options = {
    hAxis: {title: 'Date', minValue:new Date(2011, 12, 4), maxValue:new Date(2012, 11, 31), textStyle: {color: '#fff'}, titleTextStyle: {color: '#fff'}, baselineColor: 'white'},
    vAxis: {title: 'Number of detentions', textStyle: {color: '#fff'}, titleTextStyle: {color: '#fff'}, baselineColor: 'white'},
    legend: 'none',
...
function filterOvd(id, name) {
  $('<h2 class="ovheader">Detained in the police department ' + name + ' from 04.12.2011 to 31.12.2012</h2>').replaceAll('.ovheader');
