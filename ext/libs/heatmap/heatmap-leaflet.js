 ...
 L.TileLayer.HeatMap = L.TileLayer.Canvas.extend({
    ...
    _getMaxValue: function() {
        if (this.options.maxPerView) {
            ...
            this._data.forEach(function(item){
                if (mapBounds.contains([parseFloat(item.properties.lat), parseFloat(item.properties.lon)])) {
                    dataset.push(item);
                };
            });
    ...
    _calculateMaxValue: function(dataset) {
        ...
        dataset.forEach(function(item){
            array.push(item.properties.value);
        });
    ...
    _draw: function(ctx) {
        ...
        if (this._data.length > 0) {
            for (var i=0, l=this._data.length; i<l; i++) {
                ...
                var lonlat = [this._data[i].properties.lon, this._data[i].properties.lat];
